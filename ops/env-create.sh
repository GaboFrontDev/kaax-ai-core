...
_PROJECT="${PROJECT_NAME:-$(basename "$(git -C "$ROOT_DIR" rev-parse --show-toplevel 2>/dev/null || echo "$ROOT_DIR")")}"
SERVICE_NAME="${SERVICE_NAME:-${_PROJECT}-${ENV_NAME}-${AGENT_NAME}}"
SECRET_NAME="${SECRET_NAME:-${_PROJECT}/${ENV_NAME}/${AGENT_NAME}}"
